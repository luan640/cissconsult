(function () {
  ...
  document.addEventListener('click', (event) => {
    ...
    const editButton = event.target.closest('[data-open-edit-complaint-type]');
    if (editButton) {
      const editForm = document.querySelector('[data-edit-complaint-type-form]');
      if (!editForm) return;
      editForm.action = editButton.dataset.updateUrl || '';
      editForm.querySelector('#edit_complaint_label').value = editButton.dataset.label || '';
      openModal('edit-complaint-type-modal');
      return;
    }
  ...
  document.addEventListener('submit', async (event) => {
    ...
    const shouldHandle =
      form.matches('[data-complaint-type-create-form]') ||
      form.matches('[data-edit-complaint-type-form]') ||
      form.matches('[data-complaint-type-deactivate-form]');
